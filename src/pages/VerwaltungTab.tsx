...
function LagerorteSeite({ open, onBack }: { open: boolean; onBack: () => void }) {
  ...
  const handleAddRoom = async () => {
    ...
    if (res) { toast(`Raum "${rName}" hinzugefügt`); setShowAddRoom(false); setRName(''); setREmoji('🔧'); setRFloor('EG') }
    else toast('Fehler', 'err')
  }

  const handleAddSpot = async () => {
    if (!sCode.trim() || !selRaumId) return
    const res = await createLagerplatz({ raum_id: selRaumId, code: sCode.trim().toUpperCase(), bezeichnung: sLabel.trim(), regal: sRegal.trim() })
    if (res) { toast(`Lagerplatz "${res.code}" hinzugefügt`); setShowAddSpot(false); setSCode(''); setSLabel(''); setSRegal('') }
    else toast('Fehler', 'err')
  }

  const handleDelRoom = async (id: string) => {
    const cnt = lieArtikel.filter(a => a.raum_id === id).length
    if (cnt > 0) { toast(`Raum hat noch ${cnt} Artikel`, 'err'); setConfirmDelRoom(null); return }
    await deleteRaum(id)
    if (selRaumId === id) setSelRaumId(null)
    toast('Raum gelöscht'); setConfirmDelRoom(null)
  }

  if (!open) return null

  // Show room detail if selected
  if (selRaumId && selectedRaum) {
    return (
      <Page open title={selectedRaum.name} onBack={() => setSelRaumId(null)} backLabel="Lagerorte"
        right={<button type="button" className="text-red-400 text-sm" onClick={() => setConfirmDelRoom(selRaumId)}>Löschen</button>}>
        <SL>Info</SL>
        <Card><div className="px-4 py-3.5"><div className="flex gap-4"><span className="text-surface-500 text-[10px] uppercase tracking-widest font-semibold w-20 pt-0.5">Stockwerk</span><span className="text-white text-sm">{selectedRaum.stockwerk}</span></div></div>
        <div className="border-t border-surface-700/60 px-4 py-3.5"><div className="flex gap-4"><span className="text-surface-500 text-[10px] uppercase tracking-widest font-semibold w-20 pt-0.5">Artikel</span><span className="text-white text-sm">{lieArtikel.filter(a => a.raum_id === selRaumId).length}</span></div></div></Card>
        <SL>Lagerplätze</SL>
        <Card>
          {raumSpots.map((s, i) => (
            <div key={s.id} className={`flex items-center gap-3 px-4 py-3.5 ${i>0?'border-t border-surface-700/60':''}`}>
              <span className="font-bold text-sm px-2.5 py-1 rounded-xl bg-surface-700 text-brand-300">{s.code}</span>
              <div className="flex-1 min-w-0"><p className="text-white text-sm">{s.bezeichnung}</p>{s.regal && <p className="text-surface-500 text-xs">{s.regal}</p>}</div>
              <span className="text-surface-500 text-xs">{lieArtikel.filter(a => a.lagerplatz_id === s.id).length} Artikel</span>
            </div>
          ))}
          <button type="button" onClick={() => setShowAddSpot(true)} className="w-full flex items-center gap-3 px-4 py-3.5 border-t border-surface-700/60">
        ...
        <Confirm open={!!confirmDelRoom} title="Raum löschen?" body="Alle Lagerplätze werden gelöscht." onConfirm={() => handleDelRoom(confirmDelRoom!)} onCancel={() => setConfirmDelRoom(null)} />
        <Sheet open={showAddSpot} onClose={() => setShowAddSpot(false)} title="Neuer Lagerplatz">
          <div className="p-5">
            ...
            <Btn onClick={handleAddSpot} disabled={!sCode.trim()}>Hinzufügen</Btn>
        ...
        {lieRaeume.map((r, i) => {
          ...
          return (
            <CardRow key={r.id} onClick={() => setSelRaumId(r.id)} className={i>0?'border-t border-surface-700/60':''}>
              ...
              <div className="flex-1 min-w-0"><p className="text-white text-sm font-medium">{r.name}</p><p className="text-surface-500 text-xs">{sc} Lagerplätze · {cnt} Artikel · {r.stockwerk}</p></div>
              <ChevronRight size={16} className="text-surface-600 flex-shrink-0" />
            </CardRow>
          )
        })}
        <button type="button" onClick={() => setShowAddRoom(true)} className="w-full flex items-center gap-3 px-4 py-3.5 border-t border-surface-700/60 last:border-0">
          <div className="w-7 h-7 rounded-full bg-brand-600 flex items-center justify-center flex-shrink-0"><Plus size={15} className="text-white" /></div>
      ...
      <Sheet open={showAddRoom} onClose={() => setShowAddRoom(false)} title="Neuer Raum" tall>
        <div className="p-5">
          ...
          <div className="flex gap-2 mb-4">
            {['UG','EG','OG'].map(f => <button key={f} type="button" onClick={() => setRFloor(f)} className={`flex-1 py-2.5 rounded-2xl text-sm font-semibold border transition-all ${rFloor===f?'border-brand-500 bg-brand-900/40 text-brand-300':'border-surface-700 bg-surface-900 text-surface-400'}`}>{f}</button>)}
          </div>
          <Btn onClick={handleAddRoom} disabled={!rName.trim()}>Hinzufügen</Btn>
...
function KategorienSeite({ open, onBack }: { open: boolean; onBack: () => void }) {
  ...
  const handleAdd = async () => {
    ...
    if (res) { toast(`Kategorie "${name}" hinzugefügt`); setShowAdd(false); setName(''); setColorOpt(CAT_COLORS[0]) }
    else toast('Fehler', 'err')
  }
    ...
    <Page open title="Kategorien" onBack={onBack}>
        ...
        {lieKategorien.map((k, i) => {
          ...
          return (
            <div key={k.id} className={`flex items-center gap-3 px-4 py-3.5 ${i>0?'border-t border-surface-700/60':''}`}>
              <div className="w-10 h-10 rounded-2xl flex items-center justify-center text-xs font-bold flex-shrink-0" style={{ background: k.hintergrund, color: k.farbe }}>{k.name.slice(0,2).toUpperCase()}</div>
              <div className="flex-1 min-w-0"><p className="text-white text-sm font-medium">{k.name}</p><p className="text-surface-500 text-xs">{cnt} Artikel</p></div>
              <button type="button" onClick={() => setConfirmDel(k.id)} className="w-8 h-8 rounded-full flex items-center justify-center text-surface-500 hover:text-red-400"><Trash2 size={15} /></button>
            </div>
          )
        })}
        <button type="button" onClick={() => setShowAdd(true)} className="w-full flex items-center gap-3 px-4 py-3.5 border-t border-surface-700/60 last:border-0">
          <div className="w-7 h-7 rounded-full bg-brand-600 flex items-center justify-center flex-shrink-0"><Plus size={15} className="text-white" /></div>
          <span className="text-brand-400 text-sm font-medium">Kategorie hinzufügen</span>
        </button>
      </Card>
      <Confirm open={!!confirmDel} title="Kategorie löschen?" body="Die Kategorie wird entfernt." onConfirm={() => handleDel(confirmDel!)} onCancel={() => setConfirmDel(null)} />
      <Sheet open={showAdd} onClose={() => setShowAdd(false)} title="Neue Kategorie">
        <div className="p-5">
          ...
          <Btn onClick={handleAdd} disabled={!name.trim()}>Hinzufügen</Btn>
...
function BenutzerSeite({ open, onBack }: { open: boolean; onBack: () => void }) {
  const { benutzer, activeLieId, liegenschaften, activeUserId, activeUser, switchUser,
          perms, getLieRole, createBenutzer, deleteBenutzer, setBenutzerRolle, setLieAccess, setPerm } = useApp()
  ...
  const handleInvite = async () => {
    ...
    const res = await createBenutzer(
      { name: iName.trim(), email: iEmail.trim(), rolle: iRolle, initialen: initials, av_farbe },
      activeLieId!, iLieRolle
    )
    if (res) { toast(`Einladung an ${iEmail} gesendet`); setShowInvite(false); setIName(''); setIEmail(''); setIRolle('user'); setILieRolle('user') }
    else toast('Fehler', 'err')
  }

  const handleDelUser = async (id: string) => {
    if (id === activeUserId) { toast('Aktiver Benutzer kann nicht gelöscht werden', 'err'); return }
  ...
  if (selUid && sel) {
    return (
      <Page open title={sel.name} onBack={() => setSelUid(null)} backLabel="Benutzer">
        ...
        <SL>Aktiver Benutzer werden</SL>
        <Card><CardRow onClick={() => { switchUser(sel.id); toast(`${sel.name} ist jetzt aktiv`) }}>
          <div className="w-9 h-9 rounded-2xl bg-brand-900/40 flex items-center justify-center flex-shrink-0"><Users size={18} className="text-brand-400" /></div>
          <div className="flex-1"><p className="text-white text-sm font-medium">Als aktiven Benutzer einsetzen</p><p className="text-surface-500 text-xs">Wechselt den aktiven Account</p></div>
        </CardRow></Card>
        <SL>Globale Rolle</SL>
        <Card>{(['admin','user','guest'] as const).map((r, i) => (
          <CardRow key={r} onClick={() => setBenutzerRolle(sel.id, r)} className={i>0?'border-t border-surface-700/60':''}>
            <RoleBadge role={r} />
        ...
        <Card>{liegenschaften.map((l, i) => {
          ...
          return (
            <div key={l.id} className={`flex items-center gap-3 px-4 py-3 ${i>0?'border-t border-surface-700/60':''}`}>
              ...
              <select value={cur} onChange={e => { setLieAccess(sel.id, l.id, e.target.value as 'admin'|'user'|'guest'); toast('Zugriff aktualisiert') }}
        ...
        <Confirm open={!!confirmDel} title="Benutzer entfernen?" body={`"${sel.name}" wird unwiderruflich entfernt.`}
          onConfirm={() => handleDelUser(confirmDel!)} onCancel={() => setConfirmDel(null)} />
      </Page>
    )
  }
  ...
  // Role editor
  if (showRoleEditor) {
    const roleLabel = showRoleEditor === 'admin' ? 'Administrator' : showRoleEditor === 'user' ? 'Benutzer' : 'Gast'
    return (
      <Page open title={roleLabel} onBack={() => setShowRoleEditor(null)} backLabel="Benutzer">
        {showRoleEditor === 'admin' && (
          <div className="mx-4 mt-4 p-3.5 rounded-2xl border border-orange-800/40 bg-orange-950/30 flex gap-2.5">
        ...
        {PERM_GROUPS.map(g => (
          <div key={g.group}>
            <SL>{g.group}</SL>
            <Card>{g.perms.map((p, i) => {
              const on = perms[showRoleEditor]?.[p.id] ?? false
              const locked = showRoleEditor === 'admin' && ['inv_view','usr_view','role_edit'].includes(p.id)
              return (
                <div key={p.id} className={`flex items-center gap-3 px-4 py-3.5 ${i>0?'border-t border-surface-700/60':''}`}>
                  ...
                  {locked ? <Lock size={16} className="text-surface-600" /> : <Toggle on={on} onChange={v => { setPerm(showRoleEditor, p.id, v); toast('Berechtigung ' + (v?'aktiviert':'deaktiviert')) }} />}
                </div>
              )
            })}</Card>
          </div>
  ...
  return (
    <Page open title="Benutzer" onBack={onBack}
      ...
      {activeUser && (
        <>
          <SL>Aktiver Benutzer</SL>
          <div className="mx-4 bg-surface-800 rounded-3xl border border-surface-700 p-4 flex items-center gap-3">
            <Avatar initials={activeUser.initialen} color={activeUser.av_farbe} active />
            <div className="flex-1 min-w-0"><p className="text-white font-semibold text-sm">{activeUser.name}</p><p className="text-surface-500 text-xs">{activeUser.email}</p></div>
      ...
      <Card>
        {(['admin','user','guest'] as const).map((r, i) => (
          <CardRow key={r} onClick={() => setShowRoleEditor(r)} className={i>0?'border-t border-surface-700/60':''}>
            <div className="w-9 h-9 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: r==='admin'?'rgba(220,38,38,0.15)':r==='user'?'rgba(2,132,199,0.15)':'rgba(100,116,139,0.2)' }}>
              <Shield size={18} className={r==='admin'?'text-red-400':r==='user'?'text-brand-400':'text-surface-400'} />
            </div>
            <div className="flex-1 min-w-0"><p className="text-white text-sm font-medium">{r==='admin'?'Administrator':r==='user'?'Benutzer':'Gast'}</p><p className="text-surface-500 text-xs">{benutzer.filter(b=>b.rolle===r).length} Benutzer</p></div>
            <RoleBadge role={r} /><ChevronRight size={16} className="text-surface-600" />
          </CardRow>
        ))}
      </Card>

      {/* All users */}
      <SL>Alle Benutzer <span className="text-surface-600 font-normal normal-case">(Badge = Recht in aktiver Liegenschaft)</span></SL>
      <Card>
        {benutzer.map((b, i) => {
          ...
          return (
            <CardRow key={b.id} onClick={() => setSelUid(b.id)} className={i>0?'border-t border-surface-700/60':''}>
              ...
              <div className="flex-1 min-w-0"><p className="text-white text-sm font-medium truncate">{b.name}</p><p className="text-surface-500 text-xs truncate">{b.email}</p></div>
              <RoleBadge role={lieRole} />
              <ChevronRight size={15} className="text-surface-600 flex-shrink-0" />
            </CardRow>
          )
        })}
        <button type="button" onClick={() => setShowInvite(true)} className="w-full flex items-center gap-3 px-4 py-3.5 border-t border-surface-700/60">
          <div className="w-7 h-7 rounded-full bg-brand-600 flex items-center justify-center flex-shrink-0"><Plus size={15} className="text-white" /></div>
          <span className="text-brand-400 text-sm font-medium">Per Mail einladen</span>
        </button>
      </Card>
      ...
      <Sheet open={showInvite} onClose={() => setShowInvite(false)} title="Benutzer einladen" tall>
        <div className="p-5">
          ...
          <div className="bg-brand-900/30 border border-brand-800/40 rounded-2xl p-3.5 mb-4 text-brand-300 text-xs leading-relaxed">
            Die Person erhält eine Einladungs-E-Mail. Liegenschaften-spezifische Rechte können danach im Profil angepasst werden.
          </div>
          <Btn onClick={handleInvite} disabled={!iName.trim() || !iEmail.trim()}>Einladung senden</Btn>
    ...
    <>
      <div className="flex flex-col min-h-full" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
        ...
        <div className="flex-1 scroll-area pb-28">
          <SL>Aktive Liegenschaft</SL>
          <Card>
            <CardRow onClick={() => setPage('lagerorte')}>
              <div className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(5,150,105,0.15)' }}><Warehouse size={20} className="text-green-400" /></div>
              <div className="flex-1 min-w-0"><p className="text-white text-sm font-medium">Lagerorte</p><p className="text-surface-500 text-xs">{lieRaeume.length} Räume · {lieArtikel.length} Artikel</p></div>
              <ChevronRight size={16} className="text-surface-600" />
            </CardRow>
            <CardRow onClick={() => setPage('benutzer')} className="border-t border-surface-700/60">
              <div className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(124,58,237,0.15)' }}><Users size={20} className="text-violet-400" /></div>
              <div className="flex-1 min-w-0"><p className="text-white text-sm font-medium">Benutzer &amp; Rollen</p><p className="text-surface-500 text-xs">{benutzer.length} Benutzer</p></div>
              <ChevronRight size={16} className="text-surface-600" />
            </CardRow>
            <CardRow onClick={() => setPage('kategorien')} className="border-t border-surface-700/60">
              <div className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(217,119,6,0.15)' }}><Tag size={20} className="text-amber-400" /></div>
              <div className="flex-1 min-w-0"><p className="text-white text-sm font-medium">Kategorien</p><p className="text-surface-500 text-xs">{lieKategorien.length} Kategorien</p></div>
              <ChevronRight size={16} className="text-surface-600" />
            </CardRow>
          </Card>
          <SL>Export</SL>
          <Card>
            <CardRow onClick={() => toast('Excel-Export wird vorbereitet…', 'info')}>
              <div className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(5,150,105,0.15)' }}><span className="text-xl">📊</span></div>
              <div className="flex-1 min-w-0"><p className="text-white text-sm font-medium">Excel exportieren</p><p className="text-surface-500 text-xs">Alle {lieArtikel.length} Artikel als .xlsx</p></div>
              <ChevronRight size={16} className="text-surface-600" />
            </CardRow>
          </Card>
        </div>
